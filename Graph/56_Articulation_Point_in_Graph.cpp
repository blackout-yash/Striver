...
class Solution {
public:
	void dfs(int root, int par, int timer, vector <int> &mark, vector <int> &time, vector <int> &low, vector <int> &vis, vector<int> adj[]) {
		vis[root] = 1;
		...
		for (int child : adj[root]) {
			...
			dfs(child, root, timer + 1, mark, time, low, vis, adj);
	...
	vector <int> articulationPoints(int V, vector<int> adj[]) {
		...
		for (int i = 0; i < V; i++) {
			if (vis[i]) continue;
			dfs(i, -1, 1, mark, time, low, vis, adj);
		}
